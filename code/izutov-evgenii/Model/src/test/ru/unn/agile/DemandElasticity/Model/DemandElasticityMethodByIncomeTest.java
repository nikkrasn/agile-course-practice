...
public class DemandElasticityMethodByIncomeTest {
    ...
    public void calculateThrowsExceptionOnWrongTypeInput() {
        demandElasticityByIncome.calculate(new IncomeRange(), new PriceRange());
    }

    @Test(expected = IllegalArgumentException.class)
    public void calculateThrowsExceptionOnWrongTypeFirstRange() {
        demandElasticityByIncome.calculate(new PriceRange(), new IncomeRange());
    }

    @Test(expected = IllegalArgumentException.class)
    public void calculateThrowsExceptionOnWrongTypeSecondRange() {
        demandElasticityByIncome.calculate(new DemandRange(), new DemandRange());
    }
    ...
    public void canCalculateUndefinedType() {
        ...
        CoefficientDescription answer =
                (CoefficientDescription) demandElasticityByIncome.calculate(demands, incomes);
